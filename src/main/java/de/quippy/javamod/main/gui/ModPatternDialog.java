...
import de.quippy.javamod.multimedia.mod.gui.PatternImagePanel;
import de.quippy.javamod.multimedia.mod.gui.PatternImagePosition;
    ...
    private JCheckBox getFollowSongCheckBox() {
        if (followSongCheckBox == null) {
            ...
            followSongCheckBox.addItemListener(e -> {
                ...
                if ((e.getStateChange() == ItemEvent.SELECTED || e.getStateChange() == ItemEvent.DESELECTED) && getFollowSongCheckBox().isSelected())
                    ...
                    if (SwingUtilities.isLeftMouseButton(e)) {
                        ...
                        if (position != null) {
                            ...
                            if (e.getClickCount() > 1 && position.pattern != null) {
                                PatternElement element = position.pattern.getPatternElement(position.row, position.channel);
                                if ((position.column == PatternImagePosition.COLUMN_INSTRUMENT && element != null) && (myModInfoPanel != null))
                                    myModInfoPanel.showInstrument(element.getInstrument() - 1); // 0 is no Instrument...
                            }
                            e.consume();
                        }
