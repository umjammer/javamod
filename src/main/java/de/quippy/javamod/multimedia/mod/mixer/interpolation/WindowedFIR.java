...
//    private static final int WFIR_TYPE = WFIR_BLACKMANEXACT;
    ...
    private static double coef(int cnr, double ofs, double cut, int width, int type) {
        ...
        if (Math.abs(pos) < M_zEPS) {
            ...
            wc = switch (type) {
                case WFIR_HANN -> 0.50 - 0.50 * Math.cos(idl * posU);
                case WFIR_HAMMING -> 0.54 - 0.46 * Math.cos(idl * posU);
                case WFIR_BLACKMANEXACT -> 0.42 - 0.50 * Math.cos(idl * posU) + 0.08 * Math.cos(2.0 * idl * posU);
                case WFIR_BLACKMAN3T61 ->
                        0.44959 - 0.49364 * Math.cos(idl * posU) + 0.05677 * Math.cos(2.0 * idl * posU);
                case WFIR_BLACKMAN3T67 ->
                        0.42323 - 0.49755 * Math.cos(idl * posU) + 0.07922 * Math.cos(2.0 * idl * posU);
                case WFIR_BLACKMAN4T92 ->
                        0.35875 - 0.48829 * Math.cos(idl * posU) + 0.14128 * Math.cos(2.0 * idl * posU) - 0.01168 * Math.cos(3.0 * idl * posU);
                case WFIR_BLACKMAN4T74 ->
                        0.40217 - 0.49703 * Math.cos(idl * posU) + 0.09392 * Math.cos(2.0 * idl * posU) - 0.00183 * Math.cos(3.0 * idl * posU);
                case WFIR_KAISER4T ->
                        0.40243 - 0.49804 * Math.cos(idl * posU) + 0.09831 * Math.cos(2.0 * idl * posU) - 0.00122 * Math.cos(3.0 * idl * posU);
                default -> 1.0;
            };
            pos *= Math.PI;
    ...
    private static void initialize() {
        ...
        for (int cl = 0; cl < WFIR_LUTLEN; cl++) {
            ...
            gain = 1.0d / gain;
            for (int c = 0; c < WFIR_WIDTH; c++) {
                double coef = Math.floor(0.5d + scale * coefs[c] * gain);
