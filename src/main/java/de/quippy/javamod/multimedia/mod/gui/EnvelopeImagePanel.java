...
public class EnvelopeImagePanel extends JComponent {
    ...
    private void drawGrid(final Graphics g, final int top, final int left, final int width, final int height) {
        ...
        for (int i = 0; i < MAX_WIDTH; i++) {
            ...
            } else if ((i % (SMALLESTGRID * 4)) == 0) {
                g.setColor(GRIDSUB1_COLOR);
                g.drawLine(x, 0, x, height);
            } else if ((i % SMALLESTGRID) == 0) {
    ...
    private void drawEnvelope(final Graphics g) {
        ...
        if (envelope != null) {
            ...
            if (envelope.loop)
                drawLoopLine(g, envelope.loopStartPoint, envelope.loopEndPoint, width, height, LOOP_COLOR);
            if (envelope.sustain)
                drawLoopLine(g, envelope.sustainStartPoint, envelope.sustainEndPoint, width, height, SUSTAINLOOP_COLOR);
        }
    }
