...
public class SampleInstrumentPlayer {
    ...
    private void playSample() {
        ...
        while (!aktMemo.instrumentFinished) {
            ...
            if (aktMemo.noteFade && (aktMemo.fadeOutVolume <= 0 || (aktMemo.actVolumeLeft <= 0 && aktMemo.actRampVolRight <= 0)))
                aktMemo.instrumentFinished = true;
            ...
            for (int s = 0; s < samplesPerTick; s++) {
                ...
                long sampleL = leftBuffer[s];
                leftBuffer[s] = 0;
                long sampleR = rightBuffer[s];
                rightBuffer[s] = 0;
