...
public class SIDInfoPanel extends JPanel {
    ...
    public void fillInfoPanelWith(SidTuneInfo sidTuneInfo) {
        ...
        getSIDAuthor().setText(sidTuneInfoStrings[1]);
        getSIDCopyright().setText(sidTuneInfoStrings[2]);
        if (sidTuneInfo.getInfoString().size() > 3) {
            StringBuilder sb = new StringBuilder();
            for (int i = 3; i < sidTuneInfo.getInfoString().size(); i++)
                sb.append(sidTuneInfoStrings[i]).append('\n');
            getSIDSongInfo().setText(sb.toString());
        }
        String[] sidCommentStrings = sidTuneInfo.getCommentString().toArray(String[]::new);
        if (sidTuneInfo.getCommentString() != null && sidCommentStrings.length > 0) {
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < sidTuneInfo.getCommentString().size(); i++)
        ...
        getSIDPlayAddr().setText('$' + ModConstants.getAsHex(sidTuneInfo.getPlayAddr(), 4));
        getSIDBaseAddr1().setText('$' + ModConstants.getAsHex(sidTuneInfo.getSIDChipBase(1), 4));
        getSIDBaseAddr2().setText('$' + ModConstants.getAsHex(sidTuneInfo.getSIDChipBase(2), 4));
        getSIDFormatString().setText(sidTuneInfo.getAudioTypeString()); // TODO formatString
        getSIDDataFileName().setText(sidTuneInfo.getCompatibility().name()); // TODO dataFileName
